Page({
  ...
  normalizeMovieItem(movie = {}, index = 0) {
    ...
    const rating = typeof movie.rating === 'number' ? movie.rating.toFixed(1) : (movie.rating || '暂无评分');
    ...
    return {
      ...
      name: movie.name || movie.title || '未命名电影',
      rating,
      cover_url: coverUrl,
      coverUrl,
      genres,
  ...
  handleNegativeFeedback(e) {
    ...
    wx.showModal({
      title: '减少此类推荐',
      content: `要减少与“${movie.title}”相似的推荐吗？`,
      success: (res) => {
        if (!res.confirm) {
          return;
        }
  ...
  addToWatchlist(e) {
    ...
    const movie = this.getMovieById(movieId);
    if (!movie) {
      wx.showToast({ title: '未找到该电影', icon: 'none' });
      return;
    }
